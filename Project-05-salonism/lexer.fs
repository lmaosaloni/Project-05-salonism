...
module lexer =
  ...
  let rec private lexer nextc input tokens = 
      ...
      |  _  when List.contains nextc ['0'..'9'] ->
               ...
               if (lookahead = '.') then
                 let (tempLiteral, tempLookAhead) = collectIntLiteral (nextChar input) input (string lookahead)
                 lexer tempLookAhead input (("real_literal:" + literal + tempLiteral) :: tokens)
               else
